...
def _draw_corner_brackets(out, x1: int, y1: int, x2: int, y2: int, color):
    w = x2 - x1
    h = y2 - y1
    if w <= 0 or h <= 0:
        return
    ...
    # Top-left
    cv2.line(out, (x1, y1), (x1 + corner, y1), color, _BBOX_THICKNESS)
    cv2.line(out, (x1, y1), (x1, y1 + corner), color, _BBOX_THICKNESS)
    # Top-right
    cv2.line(out, (x2, y1), (x2 - corner, y1), color, _BBOX_THICKNESS)
    cv2.line(out, (x2, y1), (x2, y1 + corner), color, _BBOX_THICKNESS)
    # Bottom-left
    cv2.line(out, (x1, y2), (x1 + corner, y2), color, _BBOX_THICKNESS)
    cv2.line(out, (x1, y2), (x1, y2 - corner), color, _BBOX_THICKNESS)
    # Bottom-right
    cv2.line(out, (x2, y2), (x2 - corner, y2), color, _BBOX_THICKNESS)
    cv2.line(out, (x2, y2), (x2, y2 - corner), color, _BBOX_THICKNESS)
...
def _draw_cached_bboxes(frame, bbox_data):
    ...
    for obj in objects:
        ...
        _draw_corner_brackets(out, x1, y1, x2, y2, color)
        txt = f"{label} {conf:.0%}" + (f" #{track_id}" if track_id != "" else "")
        _draw_compact_label(out, txt, x1, y1, color, frame_w, frame_h)
    return out
